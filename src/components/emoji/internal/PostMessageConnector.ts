...
export type ApiUpdate = { type: string } & Record<string, unknown>;
...
class ConnectorClass<T extends InputRequestTypes> {
  ...
  private requestStatesByCallback = new Map<(...args: any[]) => void, RequestState>();
  ...
  request(messageData: RequestTypes<T>) {
    ...
    if (typeof payload.args[payload.args.length - 1] === 'function') {
      ...
      const callback = payload.args.pop() as (...args: any[]) => void;
  ...
  cancelCallback(progressCallback: CancellableCallback) {
    ...
    const { messageId } = this.requestStatesByCallback.get(progressCallback as never) || {};
    if (!messageId) {
      return;
    }
    this.postMessageOnTickEnd({
...
export function createConnector<T extends InputRequestTypes>(
  ...
  function handleMessage(ev: MessageEvent) {
    connector.onMessage(ev.data as WorkerMessageData);
  }

  worker.addEventListener('message', handleMessage);
  connector.removeMessageListener = () => worker.removeEventListener('message', handleMessage);
